...
const Button = sdkClass({
  initialize() {
    ...
    this.buttonIcon.appendChild(this.buttonImg);
  ...
  remove() {
    document.getElementById('recommend-button').remove();
  },
  show() {
...
class Recommender {
  constructor() {
    ...
    this.recData = [];
    this.onboarded = false; // set to true after onboarding info displayed
  }
  ...
  setButtonText() {
    ...
    if (customText !== undefined && customText !== '') {
      this.button.useText(customText);
    } else {
      this.button.useText('See Recommended Add-ons');
    }
  }
  ...
  setButtonImg() {
    ...
    if (customImgURL !== undefined && customImgURL !== '') {
      this.button.useIcon(customImgURL);
    } else {
      this.button.useIcon(sdkData.url('recommendation/icon.svg'));
    }
  }
